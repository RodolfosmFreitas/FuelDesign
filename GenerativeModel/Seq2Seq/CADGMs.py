...
from data_utils import fetch_minibatch, EarlyStopping, build_vocab
...
parser.add_argument('--activation', type=str, default='relu', help='Hidden layer activation, [relu, leakyrelu, elu, gelu, tanh, None=linear]')
...
# Fuel2Fuel Attention 
parser.add_argument('--num_layers', type=int, default=2, help='number of layers in the GRU ')
parser.add_argument('--hidden_size', type=int, default=256, help='GRU hidden size')
parser.add_argument('--dropout', type=float, default=0.1, help='dropout rate')
parser.add_argument('--embedded_size', type=int, default=512, help='fingerprints size')
...
# remove \xa0 is actually non-breaking space of smiles
for c, string in enumerate(SMILES):
    SMILES[c] = string.replace(u'\xa0', u' ')
fuel_list = [f for f in fuel_list if str(f) != 'nan']

SMILES = SMILES
fuel_list = fuel_list

# load tokens (pre-defined)
tokens = torch.load('tokens.pt')
# Number of tokens
n_tokens = len(tokens)
# Maximum length of the smiles
max_length = max(len(smiles) for smiles in SMILES)
...
X_train, X_test, Y_train, Y_test, fuel_train, fuel_test = train_test_split(vocab,
                                                                           CN,
                                                                           fuel_list,
...
#%% load pre-trained molecular deep learning fingerprints model
Encoder = EncoderRNN(input_size=n_tokens, 
                      hidden_size=args.hidden_size, 
                      n_layers=args.num_layers, 
                      embedding_size=args.embedded_size,
                      dropout_p=args.dropout)

print(Encoder)
# load pre-trained model 
load_dir='pre_trained_Seq2Seq/moses/fps_{}'.format(args.embedded_size)
Encoder.load_state_dict(torch.load(load_dir + "/Encoder.pt"))
...
# Model creation
# Decoder: p(y|x,z)
net_D = net_(inp_dim=args.embedded_size + args.features,
             out_dim=Y_dim,
...
# Encoder: q(z|x,y)
net_E = net_(inp_dim=args.embedded_size + Y_dim,
...
# Discriminator
net_T = net_(inp_dim=args.embedded_size + Y_dim,
             out_dim=Y_dim,
...
# Save diretory
model_dir = "Models/CADGMs_AE_{}x{}_fps_{}".\
    format(args.num_layers_AE, args.neurons_AE, args.embedded_size)
...
optimizer_G = optim.Adam(list(net_D.parameters())+list(net_E.parameters()), 
                         lr=args.lr)
...
for iter in range(1,args.n_iterations+1):
    ...
    # Fetch a batch
    X_batch, Y_batch = fetch_minibatch(X_train, Y_train, batch_size=args.batch_size)
    x = torch.LongTensor(X_batch).to(device)
    y = torch.FloatTensor(Y_batch).to(device)
    Z = torch.randn(x.shape[0], args.features, device=device)
    ...
    if iter % args.log_interval == 0:
        ...
        print('Iteration: %d, Generator_loss: %.4e, MSE: %.4e, Discriminator_loss: %.4e, Time: %.4f' % 
...
def scatter_plot_CI(y_true, y_pred, var_pred, mae):
    ...
    plt.xlabel(r'Predicted CN',fontsize=18)
    plt.ylabel(r'Measured CN',fontsize=18)
    plt.legend(loc='upper left', frameon=False, prop={'size': 18, 'weight': 'extra bold'})
    plt.title('')
...
x = torch.LongTensor(vocab).to(device)
encoder_outputs, fingers = Encoder(x)

fingers = fingers.detach().cpu().numpy()

#Z   = umap.UMAP(metric="mahalanobis", n_components=2, min_dist=0.1,n_neighbors=20, low_memory=False).fit_transform(fingers)#
pca = PCA(n_components=2)
Z   = pca.fit_transform(fingers)
Z   = MinMaxScaler().fit_transform(Z)


Y = np.zeros((CN.shape[0],1))
for idx in range(CN.shape[0]):
    mask = CN[idx] != 'nan'
    Y[idx] = np.mean(CN[idx, mask].astype(np.float64)) 
# tsne = TSNE(n_components=2)



plt.figure(figsize=(8,6), dpi=150)
plt.scatter(Z[:,0], Z[:,1], s=100, c=Y[:], cmap='jet', alpha=0.5, marker='.')
plt.box('True')
plt.grid('True')
plt.xticks(fontsize=18)
plt.yticks(fontsize=18)
plt.xlabel(r'First PC',fontsize=18)
plt.ylabel(r'Second PC',fontsize=18)
