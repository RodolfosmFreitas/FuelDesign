...
from data_utils import fetch_minibatch, EarlyStopping, remove_correlated_features
...
descriptors = pd.read_excel('Descriptors_CoOptima_Compedium.xlsx', index_col=0)

# Remove correlated descriptors using Spearman Correlation
CN_ = CN.astype(str)
mask = CN_ != 'nan'
aux_CN = np.zeros(len(CN))
for idx in range(len(CN)):
    mask = CN_[idx] != 'nan'
    aux_CN[idx] = np.mean(CN_[idx, mask].astype(float)) 

descriptors = remove_correlated_features(descriptors, aux_CN, threshold=0.95, method='spearman')
descriptors_list = list(descriptors.keys())
descriptors = descriptors.to_numpy()


# Split the data in train and test dataset
X_train, X_test, Y_train, Y_test, fuel_train, fuel_test = train_test_split(descriptors,
                                                                           CN,
                                                                           fuel_list,
...
mu_D    = X_train.mean(0)
std_D   = X_train.std(0)
X_train = (X_train - mu_D)/ std_D
X_test = (X_test - mu_D)/ std_D
...
# whole data
N_samples = 1000
X_star = (descriptors - mu_D) / std_D
X_star = torch.FloatTensor(X_star).to(device)
...
# scatter ci plot
mae = mean_absolute_error(aux_CN, mu_pred)
scatter_plot_CI(aux_CN, mu_pred, var_pred, mae)
...
# # plt.xticks(fontsize=18)
# # plt.yticks(fontsize=18)
# # plt.errorbar(np.arange(idx_train.shape[0]), 
# #               mu_pred_train[idx_train], 
# #               2*np.sqrt(var_pred_train[idx_train]), 
# #               ecolor='c',
# #               marker='s',
# #               capsize=8, 
# #               label = r'$\mu \pm 2\sigma$', 
# #               linestyle='none')
# # plt.plot(np.arange(idx_train.shape[0]), Y_star_train[idx_train], 'ks', label = "Train", markersize = 10)
# # plt.xlabel('fuels',fontsize=18)
# # plt.ylabel('Cetane Number',fontsize=18)
# # plt.legend(loc='upper left', frameon=False, prop={'size': 16, 'weight': 'extra bold'})
